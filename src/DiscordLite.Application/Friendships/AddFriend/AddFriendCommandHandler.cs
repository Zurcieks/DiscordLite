...
namespace DiscordLite.Application.Friendships.AddFriend
{
        ...
        ICurrentUser currentUser) : IRequestHandler<AddFriendCommand, string>
    {
        public async Task<string> Handle(AddFriendCommand request, CancellationToken ct)
        {
            ...
            var exists = await friendshipRepository.GetBetweenAsync(sender, receiver.Id, ct);

            if (exists != null && exists.Status == FriendshipStatus.Pending && sender == exists.SenderId)
                throw new ConflictException("Friend request already sent.");

            if (exists != null && exists.Status == FriendshipStatus.Pending && sender == exists.ReceiverId)
            ...
            var friendship = Friendship.Create(sender, receiver.Id);
