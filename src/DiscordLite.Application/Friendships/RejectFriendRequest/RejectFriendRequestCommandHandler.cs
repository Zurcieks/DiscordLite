...
public class RejectFriendRequestCommandHandler(
    ...
    ICurrentUser currentUser) : IRequestHandler<RejectFriendRequestCommand>
{
  
    public async Task Handle(RejectFriendRequestCommand request, CancellationToken ct)
    {
        ...
        if(friendship.Status != FriendshipStatus.Pending)
            throw new ConflictException("Only pending friend requests can be rejected.");
